...
const Projects = () => {
    ...
    return (
        <div className=' w-full bg-neutral-950 pb-20'>
            <p className='w-max mx-auto bg-gradient-to-r text-2xl pt-10 from-green-500 to-neutral-700 bg-clip-text text-transparent'>My Projects</p>
...
const Card = ({ item, index }: Props) => {
    return (
        <motion.div initial={{ scale: 0 }} whileInView={{ scale: 1 }} transition={{ duration: 0.5, delay: index * 0.1 }} className=' w-full flex flex-col justify-evenly text-white border border-neutral-700 p-2 rounded-xl'>
            ...
            <p className=' text-xs text-neutral-500'>{item.techs} </p>
            <a href={`${item.href}`} target="_blank" className=' m-2 h-fit border w-fit p-1 rounded-lg opacity-50'>
                <svg height="20" aria-hidden="true" fill='white' viewBox="0 0 16 16" version="1.1" width="32" data-view-component="true" className=' hover:fill-green-500 transition-all duration-200'>
